...
execute as @a if score @s morreu matches 1.. run title @a subtitle [{"text":"foi Solado ","color":"white","bold":true},{"score":{"name":"@s","objective":"totalMortes","color":"red","bold":true}},{"text":" vezes.","color":"white","bold":true}]
execute as @a if score @s morreu matches 1.. run title @a title {"text":"","extra":[{"selector":"@s","color":"gray", "bold":true}]}
